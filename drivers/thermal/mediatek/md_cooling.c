...
struct md_cooling_device *get_md_cdev(enum md_cooling_type type,
				      unsigned int pa_id)
{
	struct md_cooling_device *md_cdev;
...
static int mutt_throttle(struct md_cooling_device *md_cdev, unsigned long state)
{
	...
	if (final_state == mdc->mutt_state) {
		...
		dev_info(
			dev,
			"%s: ignore set state %ld due to final_state(%ld) is not changed\n",
	...
	msg = (final_state == MD_COOLING_UNLIMITED_STATE) ?
		      ...
		      mutt_lv_to_tmc_msg(
			      md_cdev->pa_id,
			      mdc->pdata->state_to_mutt_lv(final_state));
...
static int tx_pwr_throttle(struct md_cooling_device *md_cdev,
			   unsigned long state)
{
	struct device *dev = md_cdev->dev;
	enum md_cooling_status status;
	...
	if (state == MD_COOLING_UNLIMITED_STATE) {
		...
		if (!md_cdev->throttle_tx_power[state - 1])
			md_cdev->throttle_tx_power[state - 1] =
				default_tx_pwr[state - 1];
...
static int scg_off_throttle(struct md_cooling_device *md_cdev,
			    unsigned long state)
{
	struct device *dev = md_cdev->dev;
	enum md_cooling_status status;
	...
	msg = (state == MD_COOLING_UNLIMITED_STATE) ? scg_off_to_tmc_msg(0) :
						      scg_off_to_tmc_msg(1);
	ret = send_throttle_msg(msg);
	if (!ret) {
		md_cdev->target_state = state;
		mutex_lock(&mdc_data_lock);
...
static int md_cooling_get_max_state(struct thermal_cooling_device *cdev,
				    unsigned long *state)
{
...
static int md_cooling_get_cur_state(struct thermal_cooling_device *cdev,
				    unsigned long *state)
{
	struct md_cooling_device *md_cdev = cdev->devdata;
...
static int md_cooling_set_cur_state(struct thermal_cooling_device *cdev,
				    unsigned long state)
{
	struct md_cooling_device *md_cdev = cdev->devdata;
	int ret;
...
static int init_md_cooling_device(struct device *dev, struct device_node *np,
				  int id)
{
	...
	cdev = thermal_of_cooling_device_register(np, md_cdev->name, md_cdev,
						  &md_cooling_ops);
	if (IS_ERR(cdev))
		goto init_fail;
	md_cdev->cdev = cdev;
