...
static int board_ntc_r_to_temp(struct board_ntc_info *ntc_info, int val)
{
	...
	if (i == 0) {
		...
		temp = temp_hi +
		       mult_frac(temp_lo - temp_hi, val - r_hi, r_lo - r_hi);
	}
...
static int board_ntc_get_temp(struct thermal_zone_device *tz, int *temp)
{
	...
	if (!PTR_ERR_OR_ZERO(ntc_info->channel_charger_ntc)) {
		...
		while (count < READ_TIA_REG_COUNT_MAX) {
			...
			is_val_valid =
				is_adc_data_valid(val, tia_param->valid_bit);
			if (!is_val_valid) {
				if (ntc_info->dbg_reg && ntc_info->en_reg) {
					...
					dev_info(ntc_info->dev,
						 "TIA data invalid, 0x%x\n",
						 val);
				}
				goto RETRY;
			}

			r_type = get_tia_rc_sel(val, tia_param->rc_offset,
						tia_param->rc_mask);
...
RETRY:
	...
	v_in = ntc_info->adc_data->adc2volt(
		get_adc_data(val, tia_param->valid_bit - 1));
	...
	if (!r_ntc) {
		dev_err(ntc_info->dev,
			...
			adc_data->pullup_r[r_type], adc_data->pullup_v[r_type]);
		*temp = THERMAL_TEMP_INVALID;
	} else {
		*temp = board_ntc_r_to_temp(ntc_info, r_ntc);
	}

	dev_dbg_ratelimited(ntc_info->dev,
			    "val=0x%x, v_in/r_type/r_ntc/t=%llu/%d/%d/%d\n",
					...
					struct board_ntc_info *ntc_info)
{
	...
	ntc_info->lookup_table = devm_kcalloc(
		dev, num, sizeof(*ntc_info->lookup_table), GFP_KERNEL);
...
static int board_ntc_probe(struct platform_device *pdev)
{
	...
	ntc_info->adc_data =
		(struct pmic_auxadc_data *)of_device_get_match_data(&pdev->dev);
	...
	ntc_info->channel_charger_ntc =
		devm_iio_channel_get(&pdev->dev, "Charger_NTC");
	ntc_info->channel_flash_ntc =
		devm_iio_channel_get(&pdev->dev, "Flash_LED_NTC");
	...
	tz_dev = devm_thermal_of_zone_register(&pdev->dev, 0, ntc_info,
					       &board_ntc_ops);
	if (IS_ERR(tz_dev)) {
		ret = PTR_ERR(tz_dev);
