 ...
 *==================================================
 */
static int backlight_throttle(struct backlight_cooling_device *backlight_cdev,
			      unsigned long state)
{
					   ...
					   unsigned long state)
{
	...
	if (WARN_ON(state > backlight_cdev->max_state ||
		    !backlight_cdev->throttle))
		return -EINVAL;
...
static int init_bl_cooling_device(struct device *dev,
				  struct backlight_cooling_device *bl_cdev)
{
	...
	cdev = thermal_of_cooling_device_register(
		dev->of_node, bl_cdev->name, bl_cdev, &backlight_cooling_ops);
	if (IS_ERR(cdev))
		goto init_fail;
	bl_cdev->cdev = cdev;
