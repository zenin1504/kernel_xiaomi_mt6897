...
static const unsigned int g_lvts_device_addrs[NUM_LVTS_DEVICE_REG] = {
	RG_TSFM_DATA_0,	 RG_TSFM_CTRL_1, RG_TSV2F_CTRL_0,
	RG_TSV2F_CTRL_4, RG_TEMP_DATA_0, RG_RC_DATA_0,
	RG_DIV_DATA_0,	 RG_DBG_FQMTR,	 RG_DID_LVTS
};
...
static const unsigned int g_lvts_controller_addrs[NUM_LVTS_CONTROLLER_REG] = {
	LVTSMONCTL0_0,	 LVTSMONCTL1_0,	 LVTSMONCTL2_0, LVTSMONINT_0,
	LVTSMONINTSTS_0, LVTSMONIDET3_0, LVTSMSRCTL0_0, LVTSMSRCTL1_0,
	LVTS_ID_0,	 LVTS_CONFIG_0,	 LVTSEDATA00_0, LVTSEDATA01_0,
	LVTSEDATA02_0,	 LVTSEDATA03_0,	 LVTSATPTGT_0,	LVTSGSLOPE_0,
	LVTSOVSP0_0,	 LVTSOVSP1_0,	 LVTSOVSP2_0,	LVTSOVSP3_0,
	LVTSMSR0_0,	 LVTSMSR1_0,	 LVTSMSR2_0,	LVTSMSR3_0,
	LVTSRDATA0_0,	 LVTSRDATA1_0,	 LVTSRDATA2_0,	LVTSRDATA3_0,
	LVTSPROTCTL_0,	 LVTSPROTTC_0,	 LVTSCLKEN_0,	LVTSSPARE3_0
};
...
static unsigned int lvts_temp_to_raw_v1(struct formula_coeff *co,
					unsigned int sensor_id, int temp)
{
	unsigned int msr_raw = 0;

	msr_raw = ((long long)(((long long)co->golden_temp * 500 - co->a[0] -
				temp))
		   << 14) /
		  (-1 * co->a[0]);
...
static unsigned int lvts_temp_to_raw_v2(struct formula_coeff *co,
					unsigned int sensor_id, int temp)
{
	unsigned int msr_raw = 0;

	msr_raw = ((long long)co->a[sensor_id] << 14) /
		  (temp - (co->golden_temp * 500) + co->a[sensor_id]);
...
static int __used lvts_read_all_tc_temperature(struct lvts_data *lvts_data,
					       bool in_isr)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			msr_raw = lvts_read_tc_msr_raw(LVTSMSR0_0 + base +
						       0x4 * j);

			if (msr_raw == 0)
				current_temp = THERMAL_TEMP_INVALID;
			else
				current_temp = ops->lvts_raw_to_temp(
					&(tc[i].coeff), j, msr_raw);
...
static int lvts_read_tc_temperature(struct lvts_data *lvts_data,
				    unsigned int tz_id, bool in_isr)
{
	struct tc_settings *tc = lvts_data->tc;
...
static int soc_temp_lvts_read_temp(struct thermal_zone_device *tz,
				   int *temperature)
{
	...
	if (lvts_tz->id == 0)
		...
		*temperature =
			lvts_read_tc_temperature(lvts_data, lvts_tz->id, false);
	else
		return -EINVAL;
...
static void disable_sensing_points(struct lvts_data *lvts_data,
				   unsigned int tc_id)
{
	void __iomem *base;
...
static void enable_sensing_points(struct lvts_data *lvts_data,
				  unsigned int tc_id)
{
	struct device *dev = lvts_data->dev;
	struct tc_settings *tc = lvts_data->tc;
	...
	if (num > ALL_SENSING_POINTS) {
		dev_info(dev, "%s, LVTS%d, illegal number of sensors: %d\n",
...
static void read_controller_reg_before_active(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < NUM_LVTS_CONTROLLER_REG; j++) {
			temp = readl(LVTSMONCTL0_0 +
				     g_lvts_controller_addrs[j] + base);
...
static int lvts_write_device_reg(struct lvts_data *lvts_data,
				 unsigned int config, unsigned int dev_reg_idx,
				 unsigned int data, int tc_id)
{
	void __iomem *base;
...
static void read_device_reg_when_error(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < NUM_LVTS_DEVICE_REG; j++) {
			...
			lvts_write_device_reg(lvts_data, 0x81020000, addr, 0x00,
					      i);
 ...
 */
static int
lvts_thermal_check_all_sensing_point_idle(struct lvts_data *lvts_data)
{
...
static void set_polling_speed(struct lvts_data *lvts_data, int tc_id)
{
	...
	lvtsMonCtl1 = (((tc[tc_id].tc_speed.group_interval_delay << 20) &
			GENMASK(29, 20)) |
...
static void disable_hw_reboot_interrupt(struct lvts_data *lvts_data, int tc_id)
{
	...
	if (lvts_data->enable_dump_log) {
		temp = temp & ~(HIGH_OFFSET3_INT_EN | HIGH_OFFSET2_INT_EN |
				HIGH_OFFSET1_INT_EN | HIGH_OFFSET0_INT_EN);

		temp = temp & ~(LOW_OFFSET3_INT_EN | LOW_OFFSET2_INT_EN |
				LOW_OFFSET1_INT_EN | LOW_OFFSET0_INT_EN);
	}
				       ...
				       int trip_point, int tc_id)
{
	...
	if (d_index == ALL_SENSING_POINTS) {
		...
		writel(config | temp, LVTSPROTCTL_0 + base);
		msr_raw = ops->lvts_temp_to_raw(&(tc[tc_id].coeff), d_index,
						trip_point);
	}
...
static int soc_temp_lvts_set_trip_temp(struct thermal_zone_device *tz, int trip,
				       int temp)
{
	struct soc_temp_tz *lvts_tz = (struct soc_temp_tz *)tz->devdata;
	struct lvts_data *lvts_data = lvts_tz->lvts_data;
	...
	if (cal_data->use_fake_efuse) {
		...
		for (i = 0; i < lvts_data->num_tc; i++) {
			if (tc[i].coeff.cali_mode == CALI_HT)
				tc[i].coeff.golden_temp =
					cal_data->default_golden_temp_ht;
			else
				tc[i].coeff.golden_temp =
					cal_data->default_golden_temp;
		}
	}
}
...
static int prepare_calibration_data(struct lvts_data *lvts_data)
{
	...
	if (offset >= size)
		return -ENOMEM;
	for (i = 0; i < lvts_data->num_sensor; i++) {
		ret = snprintf(buffer + offset, size - offset, "%d:%d:%d ", i,
	...
	if (offset >= size)
		return -ENOMEM;
	for (i = 0; i < lvts_data->num_tc; i++) {
		ret = snprintf(buffer + offset, size - offset, "%d:%d ", i,
			       tc[i].coeff.golden_temp);
		if (ret < 0) {
			dev_err(dev, "%s, %d, encode wrong\n", __func__,
				__LINE__);
...
static enum interrupt_type
interrupt_type_switch(struct lvts_data *lvts_data, unsigned int tc_id,
		      unsigned int interrupt_reg_status)
{
	void __iomem *base;
			   ...
			   char thermintst_str[])
{
	...
	if (IS_ENABLE(FEATURE_SCP_OC)) {
		if (tc_id == SCP_APPOINTED_CONTROLLER) {
			...
			ignore_dump_log =
				interrupt_type_switch(lvts_data, tc_id, ret);
			if (ignore_dump_log == HOT || ignore_dump_log == COLD) {
				...
				int s_index =
					tc[tc_id]
						.sensor_map[SCP_APPOINTED_SENSOR];
				char *hotOrCold = (ignore_dump_log == HOT) ?
							  "hot" :
							  "cold";

				temp = lvts_read_tc_temperature(
					lvts_data, s_index + 1, true);
				dev_info(dev, "%s", thermintst_str);
				dev_info(
					dev,
					"it is controller%d, sensor%d %s interrupt, temp = %d\n",
					tc_id, SCP_APPOINTED_SENSOR, hotOrCold,
					temp);
#endif
	...
	dev_info(
		dev,
		"[Thermal IRQ] LVTS thermal controller %d, LVTSMONINTSTS=0x%08x, T=%d\n",
		tc_id, ret, temp);
...
static irqreturn_t irq_handler(int irq, void *dev_id)
{
	...
	if (IS_ENABLE(FEATURE_6989_SCP_OC)) {
		for (i = 0; i < lvts_data->num_tc; i++) {
			base = GET_BASE_ADDR(i);
			if (readl(LVTSMONINTSTS_0 + base) &
			    THERMAL_PROTECTION_STAGE_3) {
				disable_all_sensing_points(lvts_data);
				break;
			}
		}
	...
	for (i = 0; i < lvts_data->num_domain; i++) {
		...
		if ((!IS_ENABLE(FEATURE_SCP_OC)) &&
		    (!IS_ENABLE(FEATURE_6989_SCP_OC)))
			dev_info(dev, "%s : THERMINTST = 0x%x\n", __func__,
				 lvts_data->irq_bitmap[i]);
		else
			thermintst_str_offset += snprintf(
				thermintst_str + thermintst_str_offset,
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		if ((lvts_data->irq_bitmap[tc[i].domain_index] &
		     tc[i].irq_bit) == 0 &&
...
static int lvts_register_irq_handler(struct lvts_data *lvts_data)
{
	...
	irq_bitmap = devm_kcalloc(dev, lvts_data->num_domain,
				  sizeof(*irq_bitmap), GFP_KERNEL);
	if (!irq_bitmap) {
		dev_err(dev,
			"Error: Failed to allocate memory for irq_bitmap\n");
		return -ENOMEM;
	}
				      ...
				      struct thermal_zone_device **tzdev)
{
	...
	*tzdev = devm_thermal_of_zone_register(dev, lvts_tz->id, lvts_tz,
					       &soc_temp_lvts_ops);
...
static int lvts_register_thermal_zones(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for (j = 0; j < lvts_data->tc[i].num_sensor; j++) {
			...
			ret = lvts_register_thermal_zone(s_index + 1, lvts_data,
							 &tzdev);
			if (ret)
				return ret;
		}
				       ...
				       struct platform_device *pdev)
{
	...
	if (node) {
		if (of_property_read_u32(node, "enable-runtime-log",
					 &enable_runtime_log)) {
		...
		pr_info("[%s] Check enable_runtime_log(%d)\n", __func__,
			enable_runtime_log);
		lvts_data->enable_runtime_log =
			((enable_runtime_log == 0) ? 0 : 1);
	} else {
...
static void device_enable_and_init_v5(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US2_V4,
				  i); //0420
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US1_V4,
				  i); //0500
	...
	if (lvts_data->init_done) {
		for (i = 0; i < lvts_data->num_tc; i++) {
			...
			lvts_write_device(lvts_data,
					  SET_DEVICE_LOW_POWER_SINGLE_MODE_V4,
					  i);
		}
		return 0;
	}
...
#define SET_LVTS_MANUAL_RCK_OPERATION_V5 \
...
static int device_read_count_rc_n_v5(struct lvts_data *lvts_data)
{
	...
	if (lvts_data->init_done) {
		for (i = 0; i < lvts_data->num_tc; i++) {
			lvts_write_device(lvts_data,
					  SET_LVTS_MANUAL_RCK_OPERATION_V5,
					  i); //0E00
			lvts_write_device(lvts_data, SET_SENSOR_NO_RCK_V4,
					  i); //0d10
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_V5,
				  i);
...
static void mt6853_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6853_TS5_0] =
		(GET_CAL_DATA_BITMASK(1, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(2, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(3, 31, 24);
}
 ...
 *==================================================
 */
#define SET_TS_EN_6893 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF5)
#define SET_TS_DIS_6893 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF1)
#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6893 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8 | 0xC4)
#define SET_LANTENCY_6893 (DEVICE_WRITE | RG_TSFM_CTRL_4 << 8 | 0xA6)
#define SET_LVTS_MANUAL_RCK_OPERATION_6893 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_6 << 8 | 0x00)
...
static void mt6893_device_enable_and_init(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US2_V4, i);
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US1_V4, i);
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6893,
				  i);
		lvts_write_device(lvts_data, TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_V4,
				  i);
		lvts_write_device(lvts_data, SET_TS_RSV_V4, i);
...
static int mt6893_device_read_count_rc_n(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_6893,
				  i);
...
static void mt6893_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6893_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(21, 23, 0);

	cal_data->count_rc[MT6893_LVTS_MCU_CTRL1] =
		(GET_CAL_DATA_BITMASK(1, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(2, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(3, 31, 24);

	cal_data->count_rc[MT6893_LVTS_MCU_CTRL2] =
		(GET_CAL_DATA_BITMASK(4, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(5, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(6, 31, 24);

	cal_data->count_rc[MT6893_LVTS_AP_CTRL0] =
		(GET_CAL_DATA_BITMASK(7, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(8, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(9, 31, 24);

	cal_data->count_rc[MT6893_LVTS_AP_CTRL1] =
		(GET_CAL_DATA_BITMASK(10, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(11, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(12, 31, 24);

	cal_data->count_rc[MT6893_LVTS_AP_CTRL2] =
		(GET_CAL_DATA_BITMASK(13, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(14, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(15, 31, 24);

	cal_data->count_rc[MT6893_LVTS_AP_CTRL3] =
		(GET_CAL_DATA_BITMASK(16, 31, 24) << 16) +
		(GET_CAL_DATA_BITMASK(17, 31, 24) << 8) +
		GET_CAL_DATA_BITMASK(18, 31, 24);
}
 ...
 *==================================================
 */
#define SET_TS_DIS_6983 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF1)

#define SET_LVTS_MANUAL_RCK_OPERATION_6983 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_6 << 8 | 0x00)

#define SET_COUNTING_WINDOW_47US2 (DEVICE_WRITE | RG_TSFM_CTRL_1 << 8 | 0x47)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6983_1 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8 | 0x30)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6983_2 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_3 << 8 | 0x03)

#define TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_6983 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_4 << 8 | 0xFF)
...
static void mt6983_device_enable_and_init(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data,
				  TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6983_1, i);
		lvts_write_device(lvts_data,
				  TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6983_2, i);
		lvts_write_device(lvts_data, TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_6983,
				  i);
...
static int mt6983_device_read_count_rc_n(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_6983,
				  i);
...
static void mt6983_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6983_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(18, 23, 0);
	cal_data->count_rc[MT6983_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(19, 23, 0);
	cal_data->count_rc[MT6983_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(20, 23, 0);
	cal_data->count_rc[MT6983_LVTS_MCU_CTRL3] =
		GET_CAL_DATA_BITMASK(21, 23, 0);
	cal_data->count_rc[MT6983_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(22, 23, 0);
	cal_data->count_rc[MT6983_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(23, 23, 0);
	cal_data->count_rc[MT6983_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(24, 23, 0);
	cal_data->count_rc[MT6983_LVTS_AP_CTRL3] =
		GET_CAL_DATA_BITMASK(25, 23, 0);
	cal_data->count_rc[MT6983_LVTS_AP_CTRL4] =
		GET_CAL_DATA_BITMASK(26, 23, 0);
}
	...
	if (cal_data->use_fake_efuse) {
		...
		for (i = 0; i < lvts_data->num_sensor; i++) {
			...
			if (i >= MT6983_TS4_0 && i <= MT6983_TS4_3)
				cal_data->count_r[i] =
					DEFAULT_EFUSE_ADCT_CONTROLLER_COUNT;
		}
 ...
 *==================================================
 */
#define SET_TS_DIS_V6 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xB1)

#define SET_LVTS_MANUAL_RCK_OPERATION_V6 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_6 << 8 | 0x00)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_V6_1 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8 | 0x30)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_V6_2 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_3 << 8 | 0x03)

#define TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_V6 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_4 << 8 | 0xFF)

#define SET_DEVICE_LOW_POWER_SINGLE_MODE_V6 \
	(DEVICE_WRITE | RG_TSFM_CTRL_3 << 8 | 0xF8)
...
static void device_enable_and_init_v6(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US1_V4, i);
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_47US2, i);
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_V6_1,
				  i);
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_V6_2,
				  i);
		lvts_write_device(lvts_data, TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_V6,
				  i);
...
static int device_read_count_rc_n_v6(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_V6,
				  i);
		...
		lvts_write_device(lvts_data, SET_SENSOR_NO_RCK_V4, i);
		lvts_write_device(lvts_data,
				  SET_DEVICE_LOW_POWER_SINGLE_MODE_V6, i);
	}
...
static void mt6985_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6985_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(8, 23, 0);
	cal_data->count_rc[MT6985_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(9, 23, 0);
	cal_data->count_rc[MT6985_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(34, 23, 0);
	cal_data->count_rc[MT6985_LVTS_MCU_CTRL3] =
		GET_CAL_DATA_BITMASK(10, 23, 0);
	cal_data->count_rc[MT6985_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(11, 23, 0);
	cal_data->count_rc[MT6985_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(12, 23, 0);
	cal_data->count_rc[MT6985_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(13, 23, 0);
	cal_data->count_rc[MT6985_LVTS_AP_CTRL3] =
		GET_CAL_DATA_BITMASK(14, 23, 0);
	cal_data->count_rc[MT6985_LVTS_AP_CTRL4] =
		GET_CAL_DATA_BITMASK(29, 23, 0);

	cal_data->count_rc[MT6985_LVTS_GPU_CTRL0] =
		GET_CAL_DATA_BITMASK(31, 23, 0);
	cal_data->count_rc[MT6985_LVTS_GPU_CTRL1] =
		GET_CAL_DATA_BITMASK(32, 23, 0);
	cal_data->count_rc[MT6985_LVTS_GPU_CTRL2] =
		GET_CAL_DATA_BITMASK(33, 23, 0);
}
...
static void update_coef_data_v1(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			if (tc[i].coeff.cali_mode == CALI_HT &&
			    cal_data->cali_mode == 1) {
				tc[i].coeff.a[j] = COF_A_OFS_HT_V6 +
						   (COF_A_CONST_OFS_HT_V6 *
						    cal_data->count_r[s_index] /
						    COF_A_COUNT_R_GLD_HT_V6);
			} else {
				tc[i].coeff.a[j] = COF_A_OFS_V6 +
						   (COF_A_CONST_OFS_V6 *
						    cal_data->count_r[s_index] /
						    COF_A_COUNT_R_GLD_V6);
			}
			dev_info(lvts_data->dev, "%s tc[%d].coeff.a[%d]=%d\n",
				 __func__, i, j, tc[i].coeff.a[j]);
		}
	}
}
...
static void mt6897_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6897_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(19, 23, 0);
	cal_data->count_rc[MT6897_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(20, 23, 0);
	cal_data->count_rc[MT6897_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(21, 23, 0);
	cal_data->count_rc[MT6897_LVTS_MCU_CTRL3] =
		GET_CAL_DATA_BITMASK(22, 23, 0);
	cal_data->count_rc[MT6897_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(23, 23, 0);
	cal_data->count_rc[MT6897_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(24, 23, 0);
	cal_data->count_rc[MT6897_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(25, 23, 0);
	cal_data->count_rc[MT6897_LVTS_APU_CTRL0] =
		GET_CAL_DATA_BITMASK(27, 23, 0);
	cal_data->count_rc[MT6897_LVTS_GPU_CTRL0] =
		GET_CAL_DATA_BITMASK(28, 23, 0);
}
...
static void mt6989_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6989_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(22, 23, 0);
	cal_data->count_rc[MT6989_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(23, 23, 0);
	cal_data->count_rc[MT6989_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(24, 23, 0);
	cal_data->count_rc[MT6989_LVTS_MCU_CTRL3] =
		GET_CAL_DATA_BITMASK(25, 23, 0);

	cal_data->count_rc[MT6989_LVTS_APU_CTRL0] =
		GET_CAL_DATA_BITMASK(26, 23, 0);
	cal_data->count_rc[MT6989_LVTS_APU_CTRL1] =
		GET_CAL_DATA_BITMASK(27, 23, 0);
	cal_data->count_rc[MT6989_LVTS_GPU_CTRL0] =
		GET_CAL_DATA_BITMASK(28, 23, 0);

	cal_data->count_rc[MT6989_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(29, 23, 0);
	cal_data->count_rc[MT6989_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(30, 23, 0);
}
...
static void mt6989_update_coef_data(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			tc[i].coeff.a[j] =
				COF_A_OFS_6989 + (COF_A_CONST_OFS_6989 *
						  cal_data->count_r[s_index] /
						  COF_A_COUNT_R_GLD_6989);
 ...
 *==================================================
 */
#define SET_TS_DIS_6895 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xB1)

#define SET_LVTS_MANUAL_RCK_OPERATION_6895 \
	(DEVICE_WRITE | RG_TSV2F_CTRL_6 << 8 | 0x00)
...
static int mt6895_device_read_count_rc_n(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_6895,
				  i);
...
static void mt6895_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6895_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(18, 23, 0);
	cal_data->count_rc[MT6895_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(19, 23, 0);
	cal_data->count_rc[MT6895_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(20, 23, 0);
	cal_data->count_rc[MT6895_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(22, 23, 0);
	cal_data->count_rc[MT6895_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(23, 23, 0);
	cal_data->count_rc[MT6895_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(24, 23, 0);
	cal_data->count_rc[MT6895_LVTS_AP_CTRL3] =
		GET_CAL_DATA_BITMASK(25, 23, 0);
	cal_data->count_rc[MT6895_LVTS_AP_CTRL4] =
		GET_CAL_DATA_BITMASK(26, 23, 0);
}
...
static void mt6878_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6878_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(15, 23, 0);
	cal_data->count_rc[MT6878_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(16, 23, 0);
	cal_data->count_rc[MT6878_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(17, 23, 0);
	cal_data->count_rc[MT6878_LVTS_MCU_CTRL3] =
		GET_CAL_DATA_BITMASK(18, 23, 0);

	cal_data->count_rc[MT6878_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(19, 23, 0);
	cal_data->count_rc[MT6878_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(20, 23, 0);
	cal_data->count_rc[MT6878_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(21, 23, 0);

	cal_data->count_rc[MT6878_LVTS_GPU_CTRL0] =
		GET_CAL_DATA_BITMASK(22, 23, 0);
	/* GET_CAL_DATA_BITMASK(23, 23, 0); */
}
...
static void mt6878_update_coef_data(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			tc[i].coeff.a[j] =
				COF_A_OFS_6878 + (COF_A_CONST_OFS_6878 *
						  cal_data->count_r[s_index] /
						  COF_A_COUNT_R_GLD_6878);
...
static void mt6879_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6879_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(23, 23, 0);
...
static void mt6855_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6855_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(13, 23, 0);

	cal_data->count_rc[MT6855_LVTS_MCU_CTRL1] =
		GET_CAL_DATA_BITMASK(14, 23, 0);

	cal_data->count_rc[MT6855_LVTS_MCU_CTRL2] =
		GET_CAL_DATA_BITMASK(15, 23, 0);

	cal_data->count_rc[MT6855_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(16, 23, 0);

	cal_data->count_rc[MT6855_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(17, 23, 0);

	cal_data->count_rc[MT6855_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(18, 23, 0);
}
...
enum mt6833_lvts_domain { MT6833_AP_DOMAIN, MT6833_NUM_DOMAIN };
...
enum mt6789_lvts_domain { MT6789_AP_DOMAIN, MT6789_NUM_DOMAIN };
...
static void mt6789_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6789_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(8, 23, 0);

	cal_data->count_rc[MT6789_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(9, 23, 0);

	cal_data->count_rc[MT6789_LVTS_AP_CTRL2] =
		GET_CAL_DATA_BITMASK(10, 23, 0);

	cal_data->count_rc[MT6789_LVTS_AP_CTRL3] =
		GET_CAL_DATA_BITMASK(11, 23, 0);
}
...
static void mt6886_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	...
	cal_data->count_rc[MT6886_LVTS_MCU_CTRL0] =
		GET_CAL_DATA_BITMASK(15, 31, 8);

	cal_data->count_rc[MT6886_LVTS_MCU_CTRL1] =
		(GET_CAL_DATA_BITMASK(15, 7, 0) << 16) +
		GET_CAL_DATA_BITMASK(16, 31, 16);

	cal_data->count_rc[MT6886_LVTS_MCU_CTRL2] =
		(GET_CAL_DATA_BITMASK(16, 15, 0) << 8) +
		GET_CAL_DATA_BITMASK(17, 31, 24);

	cal_data->count_rc[MT6886_LVTS_AP_CTRL0] =
		GET_CAL_DATA_BITMASK(17, 23, 0);

	cal_data->count_rc[MT6886_LVTS_AP_CTRL1] =
		GET_CAL_DATA_BITMASK(18, 31, 8);

	cal_data->count_rc[MT6886_LVTS_AP_CTRL2] =
		(GET_CAL_DATA_BITMASK(18, 7, 0) << 16) +
		GET_CAL_DATA_BITMASK(19, 31, 16);

	cal_data->count_rc[MT6886_LVTS_AP_CTRL3] =
		(GET_CAL_DATA_BITMASK(19, 15, 0) << 8) +
		GET_CAL_DATA_BITMASK(20, 31, 24);

	cal_data->count_rc[MT6886_LVTS_AP_CTRL4] =
		GET_CAL_DATA_BITMASK(20, 23, 0);
}
...
static void mt6886_update_coef_data(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			s_index = tc[i].sensor_map[j];
			if (tc[i].coeff.cali_mode == CALI_HT) {
				tc[i].coeff.a[j] = COF_A_OFS_HT_6886 +
						   (COF_A_CONST_OFS_HT_6886 *
						    cal_data->count_r[s_index] /
						    COF_A_COUNT_R_GLD_HT_6886);
				pr_info("%s: tc=%d tc[%d].coeff.a[%d]=%d cal_data->count_r[%d]=%d HT~~~\n",
					__func__, i, i, j, tc[i].coeff.a[j],
					s_index, cal_data->count_r[s_index]);
			} else {
				tc[i].coeff.a[j] = COF_A_OFS_6886 +
						   (COF_A_CONST_OFS_6886 *
						    cal_data->count_r[s_index] /
						    COF_A_COUNT_R_GLD_6886);
