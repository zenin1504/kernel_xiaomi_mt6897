 ...
 *==================================================
 */
static int charger_throttle(struct charger_cooling_device *charger_cdev,
			    unsigned long state)
{
	...
	charger_cl_data.cur_current =
		master_charger_state_to_current_limit[state];
	dev_info(dev, "%s: set lv = %ld done\n", charger_cdev->name, state);
	return 0;
}
static int charger_cooling_get_max_state(struct thermal_cooling_device *cdev,
					 unsigned long *state)
{
...
static int charger_cooling_get_cur_state(struct thermal_cooling_device *cdev,
					 unsigned long *state)
{
	struct charger_cooling_device *charger_cdev = cdev->devdata;
...
static int charger_cooling_set_cur_state(struct thermal_cooling_device *cdev,
					 unsigned long state)
{
	struct charger_cooling_device *charger_cdev = cdev->devdata;
	int ret;
...
static int cooling_state_to_charger_limit_v1(struct charger_cooling_device *chg)
{
	...
	prop_bat_chr.intval =
		master_charger_state_to_current_limit[chg->target_state];

	ret = power_supply_set_property(
		chg->chg_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
		&prop_bat_chr);
	if (ret != 0) {
	...
	ret = power_supply_set_property(
		chg->chg_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &prop_vbus);
	if (ret != 0) {
	...
	pr_notice("chr limit state %lu, chr %d, input %d, vbus %d\n",
		  chg->target_state, prop_bat_chr.intval, prop_input.intval,
		  prop_vbus.intval);
	...
	if (chg->type == DUAL_CHARGER) {
		...
		prop_s_bat_chr.intval =
			slave_charger_state_to_current_limit[chg->target_state];
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < CHARGER_STATE_NUM; i++) {
		if (i == 0)
			len += snprintf(buf + len, PAGE_SIZE - len, "[%d,", -1);
		else if (i == CHARGER_STATE_NUM - 1)
			len += snprintf(
				buf + len, PAGE_SIZE - len, "%d]\n",
				master_charger_state_to_current_limit[i] /
					1000);
		else
			len += snprintf(
				buf + len, PAGE_SIZE - len, "%d,",
...
static struct attribute *charger_cooler_attrs[] = { &charger_curr_attr.attr,
...
static int charger_cooling_probe(struct platform_device *pdev)
{
	...
	if (charger_cdev->type == DUAL_CHARGER) {
		charger_cdev->s_chg_psy =
			power_supply_get_by_name("mtk-slave-charger");
		if (charger_cdev->s_chg_psy == NULL ||
		    IS_ERR(charger_cdev->s_chg_psy)) {
			pr_info("Couldn't get s_chg_psy\n");
			return -EINVAL;
		}
	}
	...
	if (ret) {
		dev_info(&pdev->dev,
			 "failed to create charger cooler sysfs, ret=%d!\n",
			 ret);
		return ret;
	}
	...
	cdev = thermal_of_cooling_device_register(
		np, charger_cdev->name, charger_cdev, &charger_cooling_ops);
	if (IS_ERR(cdev))
		return -EINVAL;
...
static int charger_cooling_remove(struct platform_device *pdev)
{
	...
	charger_cdev =
		(struct charger_cooling_device *)platform_get_drvdata(pdev);
