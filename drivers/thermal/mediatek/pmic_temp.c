...
struct pmic_tz_data {
	...
	int (*get_cali_data)(struct device *dev, struct pmic_tz_data *tz_data);
};
			    ...
			    int val)
{
	...
	t_current = cali_data[tz_id].intercept +
		    ((cali_data[tz_id].slope1 * y_curr) /
		     (cali_data[tz_id].slope2));
...
static void pmic_get_temp_convert_params(struct pmic_temp_info *data)
{
	...
	for (i = 0; i < tz_data->sensor_num; i++) {
		...
		cali[i].slope1 =
			(100 * 1000 * 10); /* 1000 is for 0.001 degree */
		...
		vbe_t = (-1) *
			((((cali[i].o_vts) * tz_data->pullup_volt)) / 4096) *
			1000;

		if (tz_data->o_slope_sign == 0)
			cali[i].intercept = (vbe_t * 1000) /
					    (-(factor + tz_data->o_slope * 10));
		/*0.001 degree */
		else
			cali[i].intercept = (vbe_t * 1000) /
					    (-(factor - tz_data->o_slope * 10));
		...
		cali[i].intercept =
			cali[i].intercept + (tz_data->degc_cali * (1000 / 2));
...
static int __used pmic_get_temp(struct thermal_zone_device *tz, int *temp)
{
	...
	ret = iio_read_channel_processed(cali_data[pmic_tz->tz_id].iio_chan,
					 &val);
	if (ret < 0) {
				       ...
				       struct pmic_temp_info *pmic_info)
{
	...
	for (i = 0; i < tz_data->sensor_num; i++) {
		cali_data[i].iio_chan =
			devm_iio_channel_get(dev, cali_data[i].iio_chan_name);
...
static int pmic_temp_probe(struct platform_device *pdev)
{
	...
	pmic_info->efuse_data =
		(struct pmic_tz_data *)of_device_get_match_data(&pdev->dev);
	ret = pmic_temp_parse_iio_channel(&pdev->dev, pmic_info);
	if (ret < 0)
		return ret;

	ret = pmic_info->efuse_data->get_cali_data(&pdev->dev,
						   pmic_info->efuse_data);
	if (ret < 0)
		return ret;
...
static int mt6359_get_cali_data(struct device *dev,
...
static int mt6363_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	size_t len = 0;
	unsigned short *efuse_buff;
...
static int mt6366_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	size_t len = 0;
	unsigned short *efuse_buff;
...
static int mt6368_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	size_t len = 0;
	unsigned short *efuse_buff;
...
static int mt6369_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	size_t len = 0;
	unsigned short *efuse_buff;
...
static int mt6373_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	int i = 0;
	struct pmic_tz_cali_data *cali_data = tz_data->cali_data;
...
static int mt6338_get_cali_data(struct device *dev,
				struct pmic_tz_data *tz_data)
{
	unsigned short efuse_buff;
	int i = 0;
...
static struct pmic_tz_cali_data
	mt6338_cali_data[] = { [0] = {
				       .cali_factor = 1773,
				       .o_vts = 1600,
