...
#define for_each_tput_instance(i) for (i = 0; i < NR_TPUT_CALC_INSTANCE; i++)
...
static void get_tx_bytes(struct thermal_info *data)
{
	...
	for_each_tput_instance(i) data->stats[i].cur_tx_bytes = 0;
...
static void get_cpu_info(void)
{
	...
	if (is_tcm_ready) {
		...
		cpu_info.limit_powerbudget = sign_extend32(
			readl(thermal_cputcm_base + CPU_POWERBUDGET_TCM_OFFSET),
			31);
		cpu_info.LL_min_opp_hint =
			sign_extend32(readl(thermal_cputcm_base +
					    CPU_LL_MIN_OPP_HINT_TCM_OFFSET),
				      31);
		cpu_info.BL_min_opp_hint =
			sign_extend32(readl(thermal_cputcm_base +
					    CPU_BL_MIN_OPP_HINT_TCM_OFFSET),
				      31);
		cpu_info.B_min_opp_hint =
			sign_extend32(readl(thermal_cputcm_base +
					    CPU_B_MIN_OPP_HINT_TCM_OFFSET),
				      31);
		cpu_info.LL_limit_opp =
			sign_extend32(readl(thermal_cputcm_base +
					    CPU_LL_LIMIT_OPP_TCM_OFFSET),
				      31);
		cpu_info.BL_limit_opp =
			sign_extend32(readl(thermal_cputcm_base +
					    CPU_BL_LIMIT_OPP_TCM_OFFSET),
				      31);
		cpu_info.B_limit_opp = sign_extend32(
			readl(thermal_cputcm_base + CPU_B_LIMIT_OPP_TCM_OFFSET),
			31);
		cpu_info.LL_limit_freq = readl(thermal_cputcm_base +
					       CPU_LL_LIMIT_FREQ_TCM_OFFSET);
		cpu_info.BL_limit_freq = readl(thermal_cputcm_base +
					       CPU_BL_LIMIT_FREQ_TCM_OFFSET);
		cpu_info.B_limit_freq = readl(thermal_cputcm_base +
					      CPU_B_LIMIT_FREQ_TCM_OFFSET);
		cpu_info.LL_cur_freq =
			readl(thermal_cputcm_base + CPU_LL_CUR_FREQ_TCM_OFFSET);
		cpu_info.BL_cur_freq =
			readl(thermal_cputcm_base + CPU_BL_CUR_FREQ_TCM_OFFSET);
		cpu_info.B_cur_freq =
			readl(thermal_cputcm_base + CPU_B_CUR_FREQ_TCM_OFFSET);
		cpu_info.LL_max_temp = sign_extend32(
			readl(thermal_cputcm_base + CPU_LL_MAX_TEMP_TCM_OFFSET),
			31);
		cpu_info.BL_max_temp = sign_extend32(
			readl(thermal_cputcm_base + CPU_BL_MAX_TEMP_TCM_OFFSET),
			31);
		cpu_info.B_max_temp = sign_extend32(
			readl(thermal_cputcm_base + CPU_B_MAX_TEMP_TCM_OFFSET),
			31);
	} else {
		cpu_info.ttj = sign_extend32(
		...
		cpu_info.LL_min_opp_hint = sign_extend32(
			readl(thermal_csram_base + CPU_LL_MIN_OPP_HINT_OFFSET),
			31);
		cpu_info.BL_min_opp_hint = sign_extend32(
			readl(thermal_csram_base + CPU_BL_MIN_OPP_HINT_OFFSET),
			31);
		cpu_info.B_min_opp_hint = sign_extend32(
			readl(thermal_csram_base + CPU_B_MIN_OPP_HINT_OFFSET),
			31);
		cpu_info.LL_limit_opp = sign_extend32(
			readl(thermal_csram_base + CPU_LL_LIMIT_OPP_OFFSET),
			31);
		cpu_info.BL_limit_opp = sign_extend32(
			readl(thermal_csram_base + CPU_BL_LIMIT_OPP_OFFSET),
			31);
		cpu_info.B_limit_opp = sign_extend32(
			readl(thermal_csram_base + CPU_B_LIMIT_OPP_OFFSET), 31);
		cpu_info.LL_limit_freq =
			readl(thermal_csram_base + CPU_LL_LIMIT_FREQ_OFFSET);
		cpu_info.BL_limit_freq =
			readl(thermal_csram_base + CPU_BL_LIMIT_FREQ_OFFSET);
		cpu_info.B_limit_freq =
			readl(thermal_csram_base + CPU_B_LIMIT_FREQ_OFFSET);
		cpu_info.LL_cur_freq =
			readl(thermal_csram_base + CPU_LL_CUR_FREQ_OFFSET);
		cpu_info.BL_cur_freq =
			readl(thermal_csram_base + CPU_BL_CUR_FREQ_OFFSET);
		cpu_info.B_cur_freq =
			readl(thermal_csram_base + CPU_B_CUR_FREQ_OFFSET);
		cpu_info.LL_max_temp = sign_extend32(
...
{
	gpu_info.ttj =
		sign_extend32(readl(thermal_csram_base + GPU_TTJ_OFFSET), 31);
	...
	gpu_info.temp =
		sign_extend32(readl(thermal_csram_base + GPU_TEMP_OFFSET), 31);
	...
	gpu_info.ppm_limiter =
		readl(thermal_csram_base + GPU_PPM_LIMITER_OFFSET);
	gpu_info.ppm_limit_freq =
		readl(thermal_csram_base + GPU_PPM_LIMIT_OFFSET);
	gpu_info.ctt_avg_scale =
		readl(thermal_csram_base + GPU_CTT_SCALE_OFFSET);
}
...
static void get_apu_info(void)
{
	if (thermal_apu_mbox_base) {
		...
		apu_info.limit_powerbudget =
			sign_extend32(readl(thermal_apu_mbox_base +
					    APUMBOX_POWERBUDGET_OFFSET),
				      31);
		apu_info.temp = sign_extend32(
			readl(thermal_apu_mbox_base + APUMBOX_TEMP_OFFSET), 31);
		apu_info.limit_opp = sign_extend32(
			readl(thermal_apu_mbox_base + APUMBOX_LIMIT_OPP_OFFSET),
			31);
		apu_info.cur_opp = sign_extend32(
			readl(thermal_apu_mbox_base + APUMBOX_CUR_OPP_OFFSET),
			31);
	} else {
		apu_info.ttj = sign_extend32(
...
static enum hrtimer_restart thermal_trace_work(struct hrtimer *timer)
{
	...
	for_each_possible_cpu(i) {
		if (is_tcm_ready) {
			hr_info[i].temp = sign_extend32(
				readl(thermal_cputcm_base +
				      CPU_TEMP_TCM_OFFSET + 4 * i),
				31);
			hr_info[i].predict_temp = sign_extend32(
				readl(thermal_cputcm_base +
				      CPU_PREDICT_TEMP_TCM_OFFSET + 4 * i),
				31);
			hr_info[i].headroom = sign_extend32(
				readl(thermal_cputcm_base +
				      CPU_HEADROOM_TCM_OFFSET + 4 * i),
				31);
			hr_info[i].ratio =
				readl(thermal_cputcm_base +
			...
			hr_info[i].temp =
				sign_extend32(readl(thermal_csram_base +
						    CPU_TEMP_OFFSET + 4 * i),
					      31);
			hr_info[i].predict_temp = sign_extend32(
				readl(thermal_csram_base +
				      CPU_PREDICT_TEMP_OFFSET + 4 * i),
				31);
			hr_info[i].headroom = sign_extend32(
				readl(thermal_csram_base + CPU_HEADROOM_OFFSET +
				      4 * i),
				31);
			hr_info[i].ratio =
				readl(thermal_csram_base +
		...
		pr_debug(
			"[%d] temp=%d, predict_temp=%d, headroom=%d, ratio=%d\n",
			i, hr_info[i].temp, hr_info[i].predict_temp,
...
static ssize_t hr_enable_show(struct kobject *kobj, struct kobj_attribute *attr,
			      char *buf)
{
	int len = 0;
...
static ssize_t hr_period_show(struct kobject *kobj, struct kobj_attribute *attr,
			      char *buf)
{
	int len = 0;
...
static ssize_t mdtput_show(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	int len = 0;
...
static ssize_t wifitput_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
...
static struct attribute *thermal_trace_attrs[] = {
	&enable_attr.attr, &hr_enable_attr.attr, &hr_period_attr.attr,
	&mdtput_attr.attr, &wifitput_attr.attr,	 NULL
};
...
static int __init thermal_trace_init(void)
{
	...
	hrtimer_init(&thermal_trace_data.trace_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
