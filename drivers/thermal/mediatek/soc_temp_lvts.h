...
enum calibration_mode { CALI_NT, CALI_HT, ALL_CALI_MODES };
...
enum sensor_switch_status { SEN_OFF, SEN_ON };

enum interrupt_type { HOT, COLD, OTHER };
...
struct tc_settings {
	...
	enum sensor_switch_status
		sensor_on_off[ALL_SENSING_POINTS]; /* sensor on/off choice */
...
struct platform_ops {
	...
	int (*lvts_raw_to_temp)(struct formula_coeff *co, unsigned int id,
				unsigned int msr_raw);
	unsigned int (*lvts_temp_to_raw)(struct formula_coeff *co,
...
struct lvts_data {
	...
	struct thermal_zone_device
		*tz_dev; /* tz_dev of id 0 for HW reboot trip point update */
...
#define DEVICE_WRITE \
	(1 << 31 | 1 << 30 | DEVICE_ACCESS_STARTUS | 1 << 17 | WRITE_ACCESS)

#define DEVICE_READ (1 << 31 | 1 << 30 | DEVICE_ACCESS_STARTUS | 1 << 17)
#define RESET_ALL_DEVICES (DEVICE_WRITE | RG_TSFM_RST << 8 | 0xFF)
#define READ_BACK_DEVICE_ID                                                \
	(1 << 31 | 1 << 30 | BROADCAST_ID_UPDATE | DEVICE_ACCESS_STARTUS | \
	 1 << 17 | RG_DID_LVTS << 8)
