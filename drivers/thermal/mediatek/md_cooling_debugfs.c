...
#define MD_COOLING_DEBUGFS_ENTRY_RO(name)                                \
	static int md_cooling_##name##_open(struct inode *i,             \
					    struct file *file)           \
	{                                                                \
		return single_open(file, md_cooling_##name##_show,       \
				   i->i_private);                        \
	}                                                                \
                                                                         \
...
static ssize_t md_cooling_duty_ctrl_write(struct file *flip, const char *ubuf,
					  size_t cnt, loff_t *data)
{
	...
	if (no_ims) {
		...
		enable = 1;
	} else if (active >= 1 && active <= 255 && suspend >= 1 &&
		   suspend <= 255) {
...
static ssize_t md_cooling_ca_ctrl_write(struct file *flip, const char *ubuf,
					size_t cnt, loff_t *data)
{
	int val;
...
static ssize_t md_cooling_pa_ctrl_write(struct file *flip, const char *ubuf,
					size_t cnt, loff_t *data)
{
	int val;
					      ...
					      loff_t *data)
{
	...
	if ((sscanf(buf, "%d %d %d %d", &id, &tx_pwr_lv1, &tx_pwr_lv2,
		    &tx_pwr_lv3) != (MAX_NUM_TX_PWR_STATE + 1))) {
		ret = -EINVAL;
		goto err;
	}
...
int md_cooling_debugfs_init(void)
{
	...
	if (!debugfs_create_file("status", 0440, mdc_debug_dir, NULL,
				 &md_cooling_status_fops))
		goto failed;
	if (!debugfs_create_file("duty_ctrl", 0640, mdc_debug_dir, NULL,
				 &md_cooling_duty_ctrl_fops))
		goto failed;
	if (!debugfs_create_file("ca_ctrl", 0640, mdc_debug_dir, NULL,
				 &md_cooling_ca_ctrl_fops))
		goto failed;
	if (!debugfs_create_file("pa_ctrl", 0640, mdc_debug_dir, NULL,
				 &md_cooling_pa_ctrl_fops))
		goto failed;
	if (!debugfs_create_file("update_tx_pwr", 0640, mdc_debug_dir, NULL,
				 &md_cooling_update_tx_pwr_fops))
		goto failed;
