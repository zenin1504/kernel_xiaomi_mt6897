...
#define for_each_ipi_target(i) for (i = 0; i < NUM_THERMAL_IPI_TARGET; i++)
#define is_target_invalid(t) \
	(t != IPI_TARGET_ALL && t >= NUM_THERMAL_IPI_TARGET)
...
static int thermal_ipi_register(enum thermal_ipi_target target)
{
	...
	ret = mtk_ipi_register(cfg->dev, cfg->id, NULL, NULL,
			       (void *)&cfg->ack_data);
	if (ret != 0) {
...
static void thermal_ipi_send_all(unsigned int cmd,
				 struct thermal_ipi_data *thermal_data)
{
	unsigned int i;
...
static int thermal_ipi_set_throttle_enable(enum thermal_ipi_target target,
					   int enable)
{
	struct thermal_ipi_data thermal_data;
...
static ssize_t setting_store(struct kobject *kobj, struct kobj_attribute *attr,
			     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%9s %19s", name, cmd) == 2) {
		...
		if (!strncasecmp(cmd, "disable", 7)) {
			thermal_ipi_set_throttle_enable(
				(enum thermal_ipi_target)idx, 0);
		} else if (!strncasecmp(cmd, "enable", 6)) {
			thermal_ipi_set_throttle_enable(
				(enum thermal_ipi_target)idx, 1);
		} else {
...
static struct attribute *thermal_ipi_attrs[] = { &thermal_ipi_attr.attr, NULL };
...
#if IS_ENABLED(CONFIG_MTK_TINYSYS_MCUPM_SUPPORT)
	ipi_data[IPI_TARGET_MCUPM].config.dev =
		(struct mtk_ipi_device *)get_mcupm_ipidev();
#endif
