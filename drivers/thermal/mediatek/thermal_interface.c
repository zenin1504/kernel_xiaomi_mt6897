...
int scrn_genl_send_to_user(void *data, int len)
{
	...
	ret = nla_put_s32(
		skb, SCRN_A_VAL,
		((struct _SCRN_THRM_PACKAGE *)data)->main_scrn_status);
	if (ret) {
	...
	ret = nla_put_s32(skb, SCRN_A_VAL,
			  ((struct _SCRN_THRM_PACKAGE *)data)->sub_scrn_status);
	if (ret) {
		pr_info("SCRN Add 32bit signed integer attribute failed; err code: %d\n",
			ret);
		nlmsg_free(skb);
	...
	ret = nla_put_string(skb, SCRN_A_MSG,
			     ((struct _SCRN_THRM_PACKAGE *)data)->proc_name);
	if (ret) {
...
int scrn_genl_data_handler(struct sk_buff *skb, struct genl_info *info)
{
	...
	ret = scrn_genl_send_to_user((void *)&SCRN_Status,
				     sizeof(struct _SCRN_THRM_PACKAGE));
	if (ret) {
...
int scrn_generic_netlink_init(void)
{
	...
	memcpy(SCRN_Status.proc_name, scrn_netlink_check,
	       strlen(scrn_netlink_check) + 1);
	return 0;
}
			     ...
			     void *v)
{
	...
	case LED_BRIGHTNESS_CHANGED:
		if (led_conf->connector_id ==
		    mtk_drm_get_conn_obj_id_from_idx(0, 1)) {
		...
		} else if (led_conf->connector_id ==
			   mtk_drm_get_conn_obj_id_from_idx(1, 1)) {
			if (led_conf->cdev.brightness > 0) {
...
int get_thermal_headroom(enum headroom_id id)
{
	...
	if (id >= SOC_CPU0 && id < SOC_CPU0 + num_possible_cpus()) {
		if (tm_data.is_cputcm)
			headroom = therm_intf_read_cputcm_s32(
				CPU_HEADROOM_TCM_OFFSET + 4 * id);
		else
			headroom = therm_intf_read_csram_s32(
				CPU_HEADROOM_OFFSET + 4 * id);
	} else if (id == PCB_AP) {
		mutex_lock(&tm_data.lock);
		if (tm_data.is_cputcm)
			headroom = therm_intf_read_cputcm_s32(
				AP_NTC_HEADROOM_TCM_OFFSET);
		else
			headroom = therm_intf_read_csram_s32(
				AP_NTC_HEADROOM_OFFSET);
...
int set_cpu_min_opp(int gear, int opp)
{
	...
	if (tm_data.is_cputcm)
		therm_intf_write_cputcm(opp,
					CPU_MIN_OPP_HINT_TCM_OFFSET + 4 * gear);
	else
...
int get_cpu_temp(int cpu_id)
{
	...
	if (tm_data.is_cputcm)
		temp = therm_intf_read_cputcm_s32(CPU_TEMP_TCM_OFFSET +
						  4 * cpu_id);
	else
...
int get_dsu_temp(void)
{
	...
	if (tm_data.is_cputcm)
		temp = therm_intf_read_cputcm_s32(
			DSU_AVG_TEMP_BASE_ADDR_TCM_OFFSET);
	else
...
static void write_max_ttj(unsigned int cpu_max_ttj, unsigned int gpu_max_ttj,
...
{
	if (tm_data.is_cputcm)
		therm_intf_write_cputcm(jatm_suspend,
					CPU_JATM_SUSPEND_TCM_OFFSET);
	else
...
int get_jatm_suspend(void)
{
	...
	if (tm_data.is_cputcm)
		cpu_jatm_suspend =
			therm_intf_read_cputcm_s32(CPU_JATM_SUSPEND_TCM_OFFSET);
	else
		cpu_jatm_suspend =
			therm_intf_read_csram_s32(CPU_JATM_SUSPEND_OFFSET);
...
static ssize_t ttj_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf)
{
	int len = 0;
...
static ssize_t ttj_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t max_ttj_show(struct kobject *kobj, struct kobj_attribute *attr,
			    char *buf)
{
	int len = 0;
...
static ssize_t max_ttj_store(struct kobject *kobj, struct kobj_attribute *attr,
			     const char *buf, size_t count)
{
	char cmd[10];
	unsigned int cpu_max_ttj, gpu_max_ttj, apu_max_ttj;

	if (sscanf(buf, "%8s %u %u %u", cmd, &cpu_max_ttj, &gpu_max_ttj,
		   &apu_max_ttj) == 4) {
...
static ssize_t min_ttj_show(struct kobject *kobj, struct kobj_attribute *attr,
			    char *buf)
{
	int len = 0;
...
static ssize_t min_ttj_store(struct kobject *kobj, struct kobj_attribute *attr,
			     const char *buf, size_t count)
{
	char cmd[10];
	unsigned int min_ttj;

	if (sscanf(buf, "%8s %u", cmd, &min_ttj) == 2) {
				       ...
				       const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%9s %d %d %d %d", cmd, &cluster0_min_freq,
		   &cluster1_min_freq, &cluster2_min_freq,
		   &gpu_min_freq) == 5) {
		if (strncmp(cmd, "MIN_FREQ", 8) == 0) {
			if (tm_data.is_cputcm) {
				therm_intf_write_cputcm(
					cluster0_min_freq,
					MIN_THROTTLE_FREQ_TCM_OFFSET);
				therm_intf_write_cputcm(
					cluster1_min_freq,
...
static ssize_t cpu_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
...
static ssize_t cpu_temp_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
	int cpu_id = 0;
...
static ssize_t gpu_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
...
static ssize_t gpu_freq_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
...
static ssize_t apu_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
				 ...
				 struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < tm_data.cpu_cluster_num; i++) {
		if (tm_data.is_cputcm)
			limit_opp = therm_intf_read_cputcm_s32(
				CPU_LIMIT_OPP_TCM_OFFSET + 4 * i);
		else
			limit_opp = therm_intf_read_csram_s32(
				CPU_LIMIT_OPP_OFFSET + 4 * i);
...
static ssize_t frs_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0;
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			...
			frs_data.enable, frs_data.activated, frs_data.pid,
			frs_data.target_fps, frs_data.diff, frs_data.tpcb,
			frs_data.tpcb_slope, frs_data.ap_headroom,
			frs_data.n_sec_to_ttpcb, frs_data.frs_target_fps,
			frs_data.real_fps, frs_data.target_tpcb,
			frs_data.ptime);

	return len;
}

static ssize_t frs_info_store(struct kobject *kobj, struct kobj_attribute *attr,
			      const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", &enable, &act,
		   &pid, &target_fps, &diff, &tpcb, &tpcb_slope, &ap_headroom,
		   &n_sec_to_ttpcb, &frs_target_fps, &real_fps, &target_tpcb,
		   &ptime) == 13) {
		if ((ap_headroom >= -1000) && (ap_headroom <= 1000)) {
			if (tm_data.is_cputcm)
				...
				therm_intf_write_csram(ap_headroom,
						       AP_NTC_HEADROOM_OFFSET);
				 ...
				 size_t count)
{
	...
	if (kstrtoint(buf, 10, &target_tpcb) == 0)
		if (tm_data.is_cputcm)
			therm_intf_write_cputcm(target_tpcb,
						TARGET_TPCB_TCM_OFFSET);
		else
				    ...
				    const char *buf, size_t count)
{
	...
	if (!md_info_data.sensor_info) {
		ts_info = devm_kcalloc(tm_data.dev, num,
				       sizeof(struct md_thermal_sensor_t),
				       GFP_KERNEL);
		if (!ts_info)
			return -ENOMEM;
...
static ssize_t md_sensor_info_show(struct kobject *kobj,
				   struct kobj_attribute *attr, char *buf)
{
	int len = 0, i;
	struct md_thermal_sensor_t *ts_info;
				      ...
				      const char *buf, size_t count)
{
	...
	if (!md_info_data.actuator_info) {
		ta_info = devm_kcalloc(tm_data.dev, num,
				       sizeof(struct md_thermal_actuator_t),
				       GFP_KERNEL);
		if (!ta_info)
			return -ENOMEM;
...
static ssize_t md_actuator_info_show(struct kobject *kobj,
				     struct kobj_attribute *attr, char *buf)
{
	int len = 0, i;
	struct md_thermal_actuator_t *ta_info;
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d",
			md_info_data.md_autonomous_ctrl);
	len += snprintf(buf + len, PAGE_SIZE - len, ",%d",
			md_info_data.actuator_num);
...
static ssize_t utc_count_show(struct kobject *kobj, struct kobj_attribute *attr,
			      char *buf)
{
	int len = 0;
				    ...
				    const char *buf, size_t count)
{
	...
	if (!kstrtoint(buf, 10, &enable)) {
		...
		if (tm_data.is_cputcm) {
			therm_intf_write_cputcm(enable,
						CG_POLICY_MODE_TCM_OFFSET);
				 ...
				 size_t count)
{
	...
	char skin_name[THERMAL_NAME_LENGTH + 1],
		ref_name[THERMAL_NAME_LENGTH + 1];
	long long ref_coef;
	...
	for (skin_id = 0; skin_id < plat_vtskin_info->num_sensor; skin_id++) {
		if (!strcmp(skin_name,
			    plat_vtskin_info->params[skin_id].tz_name))
			break;
	}
	...
	for (i = 0; i < ref_num; i++) {
		if (sscanf(buf, "%20s %lld%n", ref_name, &ref_coef, &len) !=
		    2) {
	...
	memcpy(&param->vtskin_ref[0], &coef[0],
	       sizeof(struct vtskin_coef) * MAX_VTSKIN_REF_NUM);
...
static ssize_t catm_p_show(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	int len = 0;
...
static ssize_t catm_p_store(struct kobject *kobj, struct kobj_attribute *attr,
			    const char *buf, size_t count)
{
	char cmd[10];
	int p0, p1, p2;

	if (sscanf(buf, "%5s %d %d %d", cmd, &p0, &p1, &p2) == 4) {
...
static ssize_t pid_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	int len = 0, i;
	...
	for (i = 0; i < pid_info_data.pid_num; i++) {
		...
		len += snprintf(buf + len, PAGE_SIZE - len, "%d,%d,%d,%d",
				pid_data[i].limit_state, pid_data[i].p,
				pid_data[i].i, pid_data[i].d);
	}
...
static ssize_t pid_info_store(struct kobject *kobj, struct kobj_attribute *attr,
			      const char *buf, size_t count)
{
	char cmd[4];
	...
	pid_data = pid_info_data.pid_term_data;
	for (i = 0; i < pid_info_data.pid_num; i++) {
		if (sscanf(buf, " %d %d %d %d%n", &state, &p_term, &i_term,
			   &d_term, &len) == 4) {
			buf += len;
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor0.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			u_vsensor0.temp = temp;
			len = snprintf(u_vsensor0.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor1.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			...
			len = snprintf(u_vsensor1.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
			if (len != USER_VSENSOR_NAME)
				pr_info("user_vsensor_name write fail, %d\n",
					len);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor2.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			...
			len = snprintf(u_vsensor2.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
			if (len != USER_VSENSOR_NAME)
				pr_info("user_vsensor_name write fail, %d\n",
					len);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor3.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			...
			len = snprintf(u_vsensor3.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
			if (len != USER_VSENSOR_NAME)
				pr_info("user_vsensor_name write fail, %d\n",
					len);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor4.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			...
			len = snprintf(u_vsensor4.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
			if (len != USER_VSENSOR_NAME)
				pr_info("user_vsensor_name write fail, %d\n",
					len);
				  ...
				  struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%d, %s", u_vsensor5.temp,
				   ...
				   size_t count)
{
	...
	if (sscanf(buf, "%9s %d %s", cmd, &temp, name) == 3) {
		if (strncmp(cmd, "U_VSENSOR", 9) == 0) {
			...
			len = snprintf(u_vsensor5.user_vsensor_name,
				       USER_VSENSOR_NAME, "%s", name);
			if (len != USER_VSENSOR_NAME)
				pr_info("user_vsensor_name write fail, %d\n",
					len);
...
static struct kobj_attribute md_actuator_info_attr =
	__ATTR_RW(md_actuator_info);
static struct kobj_attribute info_b_attr = __ATTR_RO(info_b);
static struct kobj_attribute utc_count_attr = __ATTR_RO(utc_count);
static struct kobj_attribute dsu_ceiling_freq_attr =
	__ATTR_RO(dsu_ceiling_freq);
...
static struct attribute *thermal_attrs[] = { &ttj_attr.attr,
...
static ssize_t emul_temp_write(struct file *flip, const char *ubuf, size_t cnt,
			       loff_t *data)
{
	int ret, temp;
	char *buf;
...
static ssize_t cpu_cooler_write(struct file *flip, const char *ubuf, size_t cnt,
				loff_t *data)
{
	int ret, len, i;
	char *buf;
	...
	len = sscanf(buf, "%d %d %d %d %d %d %d %d %d %d %d %d", &values[0],
	...
	for (i = 0; i < len; i++) {
		if (tm_data.is_cputcm)
			therm_intf_write_cputcm(values[i],
						CPU_COOLER_TCM_BASE + (i * 4));
		else
			therm_intf_write_csram(values[i],
					       CPU_COOLER_BASE + (i * 4));
	}
...
static ssize_t gpu_cooler_write(struct file *flip, const char *ubuf, size_t cnt,
				loff_t *data)
{
	int ret, value;
	char *buf;
	...
	if (sscanf(buf, "%15s %d", target, &value) != 2) {
		dev_info(tm_data.dev,
			 "invalid input for gpu cooler dbg intf\n");
		ret = -EINVAL;
		goto err;
	}
...
static ssize_t gpu_temp_debug_write(struct file *flip, const char *ubuf,
				    size_t cnt, loff_t *data)
{
	...
	if (kstrtoint(buf, 10, &value) != 0) {
		dev_info(tm_data.dev,
			 "invalid input for gpu temp check dbg intf\n");
		ret = -EINVAL;
		goto err;
	}
...
static void therm_intf_debugfs_init(void)
{
	...
	debugfs_create_file("emul_temp", 0640, tm_data.debug_dir, NULL,
			    &emul_temp_fops);
	debugfs_create_file("gpu_cooler_debug", 0640, tm_data.debug_dir, NULL,
			    &gpu_cooler_fops);
	debugfs_create_file("gpu_temp_check", 0640, tm_data.debug_dir, NULL,
			    &gpu_temp_debug_fops);
	debugfs_create_file("cpu_cooler_debug", 0640, tm_data.debug_dir, NULL,
			    &cpu_cooler_fops);

	if (tm_data.is_cputcm)
		therm_intf_write_cputcm(THERMAL_TEMP_INVALID,
					EMUL_TEMP_TCM_OFFSET);
	else
	...
	therm_intf_write_apu_mbox(THERMAL_TEMP_INVALID,
				  APU_MBOX_EMUL_TEMP_OFFSET);
}
